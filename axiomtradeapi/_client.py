...
class AxiomTradeClient:
    def __init__(
        ...
        if not has_credentials and not has_tokens:
            raise ValueError(
                "Either username/password or auth_token/refresh_token must be provided"
            )
        ...
        self.ws = AxiomTradeWebSocketClient(
            auth_manager=self.auth_manager, log_level=log_level
        )
    ...
    def is_authenticated(self) -> bool:
        ...
        """
        return self.auth_manager.tokens is not None and bool(
            self.auth_manager.tokens.access_token
        )
    ...
    async def GetTokenPrice(self, token_symbol: str) -> Optional[float]:
        ...
        try:
            ...
            token_subscribe = await self.ws.subscribe_token_price(
                token_symbol, lambda data: data.get("price")
            )
            if not token_subscribe:
                self.logger.error(
                    f"Failed to subscribe to token price for {token_symbol}"
                )
                return None
    ...
    def GetBatchedBalance(
        self, wallet_addresses: List[str]
    ) -> Dict[str, Dict[str, Union[float, int]]]:
        """Get balances for multiple wallet addresses in a single request."""
        try:
            payload = {"publicKeys": wallet_addresses}

            self.logger.debug(
                f"Sending batched balance request for wallets: {wallet_addresses}"
            )
            ...
            if response.status_code == 200:
                ...
                for address in wallet_addresses:
                    if address in response_data:
                        ...
                        self.logger.info(
                            f"Successfully retrieved balance for {address}: {sol} SOL"
                        )
                    else:
                        self.logger.warning(
                            f"No balance data received for address: {address}"
                        )
                        result[address] = None
    ...
    def buy_token(
        ...
        try:
            ...
            self.logger.info(
                f"Initiating buy order for {amount_sol} SOL worth of token {token_mint}"
            )

            # Get transaction from API
            url = f"{self.base_url_api}{self.endpoints.ENDPOINT_BUY_TOKEN}"
            response = self.auth_manager.make_authenticated_request(
                "POST", url, json=buy_data
            )
    ...
    def sell_token(
        ...
        try:
            ...
            self.logger.info(
                f"Initiating sell order for {amount_tokens} tokens of {token_mint}"
            )

            # Get transaction from API
            url = f"{self.base_url_api}{self.endpoints.ENDPOINT_SELL_TOKEN}"
            response = self.auth_manager.make_authenticated_request(
                "POST", url, json=sell_data
            )
    ...
    def _sign_and_send_transaction(
        self, keypair: Keypair, transaction_data: Dict
    ) -> Dict[str, Union[str, bool]]:
        ...
        try:
            ...
            send_data = {
                "signedTransaction": base64.b64encode(bytes(signed_transaction)).decode(
                    "utf-8"
                )
            }
            ...
            if response.status_code == 200:
                ...
                self.logger.info(
                    f"Transaction sent successfully. Signature: {signature}"
                )
                return {
                    "success": True,
                    "signature": signature,
    ...
    def get_token_balance(
        ...
        """
        try:
            payload = {"publicKey": wallet_address, "tokenMint": token_mint}
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Failed to get token balance: {response.status_code}"
                )
                return None
    ...
    async def subscribe_sol_price(self, callback):
        """Subscribe to sol price updates via WebSocket."""
        return await self.ws.subscribe_sol_price(callback)
    ...
    def login_step1(self, email: str, b64_password: str) -> str:
        ...
        data = {"email": email, "b64Password": b64_password}
    ...
    def login_step1_try_servers(
        self, email: str, b64_password: str, otp_token: str = ""
    ) -> str:
        """
    ...
    def login_step2(
        self, otp_jwt_token: str, otp_code: str, email: str, b64_password: str
    ) -> Dict:
        """
        ...
        data = {"code": otp_code, "email": email, "b64Password": b64_password}
    ...
    def get_trending_tokens(self, access_token: str, time_period: str = "1h") -> Dict:
        ...
        result = response.json()
        self.logger.debug(
            f"Retrieved {len(result) if isinstance(result, list) else 'N/A'} trending tokens"
        )
        return result
